...
export function Integrations({ className }: { className?: string }) {
  ...
  const div3Ref = useRef<HTMLDivElement>(
    null,
  ) as React.RefObject<HTMLDivElement>;
  const div4Ref = useRef<HTMLDivElement>(
    null,
  ) as React.RefObject<HTMLDivElement>;
  const div5Ref = useRef<HTMLDivElement>(
    null,
  ) as React.RefObject<HTMLDivElement>;
  const div6Ref = useRef<HTMLDivElement>(
    null,
  ) as React.RefObject<HTMLDivElement>;
  const div7Ref = useRef<HTMLDivElement>(
    null,
  ) as React.RefObject<HTMLDivElement>;
  ...
  return (
    <div
      className={cn(
        "relative flex w-full max-w-125 items-center justify-center overflow-hidden rounded-lg border bg-background p-10 md:shadow-xl",
        className,
      )}
      ref={containerRef}
    >
